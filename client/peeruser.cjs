...
export class PeerUser extends Emitter {
            ...
            switch(event) {
                case 'handshake':
                    if(data.data.step === 'accept') {
                        this.connection.send({uid: this.peerServer._id, event: 'handshake', data: {step: 'complete'}});
                        this.setState('peerconnected');
                    }
                    break;
            }
    ...
    initiatePeerHandshake() {
        this.connection.send({uid: this.peerServer._id, event: 'handshake', data: {step: 'start'}});
    }
